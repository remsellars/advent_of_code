class IntersectionCalculator
  attr_accessor :path1, :path2, :intersections
  ...
  def self.part1
    ...
    calc.closest_to_origin.distance_from_origin
  end
  ...
  def least_steps()
    step_counts = intersections.map { |i| @path1.step_count(i) + @path2.step_count(i) }
    step_counts.min()
  end

  def find_intersections()
    # I'm having to map the coord into a string and back becaause I can't get & to work with arrays of coords
    coords1 = @path1.coordinates.drop(1).map { |c| c.to_s}
    coords2 = @path2.coordinates.drop(1).map { |c| c.to_s}

    intersections = coords1 & coords2
    intersections.map { |c| Coordinate.from_string(c)}
  end
...
class Coordinate
  ...
  def self.from_string(string)
    coords = string.split(",")
    Coordinate.new(coords[0].to_i, coords[1].to_i)
  end
